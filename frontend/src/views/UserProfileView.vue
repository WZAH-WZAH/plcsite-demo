<script setup>
  ...
  async function onBannerChange(e) {
    ...
    try {
      ...
      await auth.loadMe()
      ui.openModal('头图更新成功', { title: '提示' })
    } catch (e2) {
      ui.openModal(e2?.response?.data?.detail || '头图上传失败', { title: '提示' })
    } finally {
    ...
    <div v-if="user" class="space-container">
      ...
      <div class="space-nav">
        <div class="nav-inner">
          ...
          >
            <span>▶️</span> 投稿 <span class="nav-num">{{ posts.length }}</span>
          </div>
        ...
        <div v-else-if="activeTab === 'following' && isMe">
          ...
          <div class="follow-grid">
            <div v-for="u in following" :key="u.id" class="user-card" @click="router.push(`/u/${u.pid}`)">
              ...
              <div class="uc-bio">{{ u.bio || '暂无简介' }}</div>
            </div>
          </div>
